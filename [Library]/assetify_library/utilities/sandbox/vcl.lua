    ...
    if not parser.isType or (parser.isType == "bool") then
        for i = 1, table.length(vcl.private.types.bool) do
            ...
            if string.sub(buffer, parser.ref, parser.ref + string.len(j) - 1) == j then
                parser.ref, parser.value, parser.isType, parser.isTypeParsed, parser.isValueSkipAppend = parser.ref + string.len(j) - 1, j, "bool", true, true
        ...
        if (not parser.isTypeChar and vcl.private.types.string[rw]) or parser.isTypeChar then
            ...
            elseif not parser.isTypeParsed and (rw == parser.isTypeChar) then
                if vcl.private.fetchRW(buffer, parser.ref + 1) == vcl.private.types.init then
                    parser.ref, parser.value, parser.isType, parser.isTypeChar, parser.isValueSkipAppend = parser.ref - string.len(parser.value) - 1, "", "object", nil, true
                elseif (vcl.private.fetchRW(buffer, parser.ref + 1) ~= vcl.private.types.space) and (vcl.private.fetchRW(buffer, parser.ref + 1) ~= vcl.private.types.newline) then return false
                    ...
                    if vcl.private.types.string[rw] and (imports.type(vcl.private.types.string[rw]) == "table") and vcl.private.types.string[rw] then
                        local queryValue = ""
                        local startIndex = string.find(parser.value, vcl.private.types.string[rw][1], startIndex)
                        queryValue = string.sub(parser.value, 0, (startIndex and (startIndex - 1)) or string.len(parser.value))
                        while(startIndex) do
                            local endIndex = string.find(parser.value, vcl.private.types.string[rw][2], startIndex)
                            ...
                            startIndex = string.find(parser.value, vcl.private.types.string[rw][1], endIndex)
...
function vcl.private.parseReturn(parser, buffer)
    if parser.isParsed then
        ...
        if not parser.isErrored or (parser.isErrored == 1) then
            parser.error = string.format(parser.error, vcl.private.fetchLine(buffer, parser.ref), (parser.isType and ("Malformed "..parser.isType)) or "Invalid declaration")
